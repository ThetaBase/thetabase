...
durability: ## Gate M8.5 — Control Plane state survives a restart (needs a Postgres)
	...
	THETA_REQUIRE_LIVE=1 cargo test -p theta-control --test restart_durability -- --nocapture
	...
	THETA_REQUIRE_LIVE=1 cargo test -p theta-control --test platform_durability -- --nocapture
...
kms: ## Gate M8.5 — the KMS wrapper, against a KMS (needs LocalStack)
	...
	THETA_REQUIRE_LIVE=1 cargo test -p theta-control --features kms --test kms_wrapper -- --nocapture
...
identity: ## Gate M4 — token isolation, OAuth, and revocation propagation
	...
	cargo test -p theta-control --test github_webhook
	cargo test -p theta-control --test preview_lifecycle
	cargo test -p theta-control --test github_rest
...
assist: ## Gate M9 — Assist is separate, cannot execute, and its output is safe by construction
	...
	cargo test -p theta-assist --test service -- --nocapture
	...
	cargo build -q -p theta-assist --example assist_scripted
	cd sdk/typescript && npm ci --silent && npx tsc -p tsconfig.json
...
claims: ## Gate M11.5 — nothing on the site outlives the spec that stated it
	...
	# to let drift.
	cargo run -q -p theta-claims -- emit-llms > /dev/null
	cargo run -q -p theta-claims -- emit-llms-full > /dev/null
