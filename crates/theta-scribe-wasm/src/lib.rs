...
// `rename_all` on a tagged enum renames the *variants*, not their fields —
// `rename_all_fields` is what renames the fields. Without it `minRisk` and
// `changeId` were rejected as unknown while the error messages in this file
// named them in camelCase, so a host following the documentation could not call
// `audit` at all. The wire is camelCase everywhere else; so is this now.
