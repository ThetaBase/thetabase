...
/// its core dumps off.
///
/// Re-exported from `theta-core` rather than declared here: the Control Plane
/// sets this variable and this crate reads it, and a second copy of the string
/// is a second chance for them to disagree.
pub use theta_core::DATA_KEY_ENV;
...
impl DataKey {
    ...
    pub fn from_hex(hex: &str) -> Result<Self> {
        ...
        for (i, byte) in key.iter_mut().enumerate() {
            *byte = u8::from_str_radix(&hex[i * 2..i * 2 + 2], 16)
                .map_err(|_| StorageError::Encryption("a data key must be hexadecimal".into()))?;
        }
    ...
    pub fn seal(&self, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        let ciphertext = self
            ...
            .map_err(|_| StorageError::Encryption("could not encrypt a record".into()))?;
