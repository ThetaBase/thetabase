...
impl Harness {
    ...
    /// A token that also carries a session public key, so the server will check
    /// this session's signatures.
    ///
    /// The private half stays with the caller, which is the entire point: a
    /// signature this server could have produced proves nothing about who wrote
    /// an entry (`04-threat-model-security.md` §7.2).
    pub fn signing_token(&self, public_key_hex: &str) -> String {
        let mut scope = scope("test-project", "dev");
        scope.signing_key = Some(public_key_hex.to_string());
