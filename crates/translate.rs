...
pub fn to_wire(host: &HostRequest) -> Result<RequestBody, String> {
    Ok(match host {
        ...
        HostRequest::Put { key, value } => RequestBody::Put {
            key: key.clone(),
            // Canonical JSON, produced here rather than by the host: two hosts
            // serialising the same value differently would give the same row
            // two representations on the wire.
            value_json: serde_json::to_string(value).map_err(|e| e.to_string())?,
            ttl: 0,
        },
...
fn plan(
    ...
    let mut context_vars: Vec<(String, String)> = params
        ...
        .map(|(k, v)| {
            Ok((
                k.clone(),
                serde_json::to_string(v).map_err(|e: serde_json::Error| e.to_string())?,
            ))
        })
